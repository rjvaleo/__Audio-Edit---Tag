...
/// The card's controls never cover the picture, in any state.
///
/// **In the dock a `.room-edit` is `position: absolute; inset: 0`.** It covers
/// the whole visual cell, by design, because it *is* the editing overlay — and
/// there is only ever meant to be one. A second one there stacks two full-cell
/// panels and lays the card's controls over the room's; done without a `hidden`
/// class at all, as this shipped once, it covers the visualiser outright and the
/// window is simply black.
///
/// So the card's panel lives in the room workspace only, where panels are
/// ordinary blocks in a scrolling column. `roomEdit` is measured alongside it as
/// the control: it *should* cover the cell when the dock's overlay is open, and
/// a test that forbade that would be describing a different program.
test('the card’s controls never cover the picture', async ({ page }) => {
  await page.goto('/');
  await page.waitForFunction(() => typeof setMode === 'function' && typeof rtDraw === 'function',
    { timeout: 20_000 });
  await page.evaluate(async () => {
    const folder = state.folders[0].name;
    const files = await api(`/api/files?folder=${encodeURIComponent(folder)}`);
  ...
  const cover = () => page.evaluate(() => {
    ...
    return { card: over('textEdit'), room: over('roomEdit'), where: state.mode, open: roomEdit.on };
  });

  const step = async (fn) => { await page.evaluate(fn); await page.waitForTimeout(350); return cover(); };

  const shut = await step(() => setMode('edit'));
  expect(shut.card, 'the card’s panel is over the dock’s visualiser').toBe(0);

  // **The state this shipped broken in twice.** The dock's editing overlay is
  // open, so a panel without a guard is laid across the whole cell.
  const open = await step(() => toggleRoomEdit());
  expect(open.card, 'the card’s panel covers the cell when the dock overlay opens').toBe(0);
  // And the control: the room's own panel is *supposed* to be there.
  expect(open.room, 'the dock’s own editor stopped covering the cell').toBeGreaterThan(1000);

  await step(() => toggleRoomEdit());
  const inRoom = await step(() => setMode('room'));
  expect(inRoom.card, 'the card’s panel is over the room').toBe(0);

  const ridge = await step(() => setVisModule('ridge'));
  expect(ridge.card, 'the card’s panel is over the ridgeline').toBe(0);

  // Released back into the dock, which is not the same as hidden again.
  const back = await step(() => setMode('edit'));
  expect(back.card, 'the card’s panel was released back over the visualiser').toBe(0);
});
