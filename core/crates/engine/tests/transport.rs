...
// The governor that used to be here is gone. It clamped the callback's *stored*
// parameters, so each block read back the value the last block had already
// reduced: a ratchet that only ever went down, and never recovered because
// `try_lock` failing meant the real setting was not re-read. Sixteen layers
// walked to one while playing, changing the layer bank mid-stream. It made the
// thing it was built to prevent very much worse.
//
// The buffer size did the work anyway -- 113% of budget to 49% on the same
// settings -- so nothing replaced it.
